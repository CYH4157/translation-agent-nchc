...
def update_model(endpoint):
    endpoint_model_map = {
        ...
        "Ollama": "llama3",
        "CUSTOM_taide": "taide-llama2-70b",
    }
    if endpoint == "CUSTOM_taide":
    ...
    with gr.Row():
        with gr.Column(scale=1) as menubar:
            endpoint = gr.Dropdown(
                label="Endpoint",
                choices=["OpenAI", "Groq", "TogetherAI", "Ollama", "CUSTOM_taide"],
                value="CUSTOM_taide",
                visible=False,                
            )
            choice = gr.Checkbox(
            ...
            model = gr.Dropdown(
                label="執行翻譯模型 (1,3輪)",
                choices=["Llama3-TAIDE-LX-8B-Chat-Alpha1","taide-llama2-70b"],
                value="taide-llama2-70b",
            )
            api_key = gr.Textbox(
                label="API_KEY",
                type="password",
...
if __name__ == "__main__":
    demo.queue(api_open=False).launch(show_api=False, share=True, server_name="0.0.0.0", server_port=7860)
